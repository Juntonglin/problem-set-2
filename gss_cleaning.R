...
df %>% ggplot(aes(x = total_children)) + geom_histogram(bins = 5)
df %>% ggplot(aes(y = total_children, x = age)) + geom_point() + labs(x = "ABC", title = "Fig 1 BBB")
#From Fig 1: ...

mod<- lm(total_children~ age+ feelings_life+ sex,data = df)
summary(mod)
#male, total_children - 0.082071; male's children is - 0.082071 from female's children

df <- df %>% filter(!is.na(feelings_life)) %>% mutate(happy = ifelse(feelings_life >= mean(df$feelings_life), 1, 0))
mod2<-glm(happy ~ age + total_children, family = binomial, data = df)
summary(mod2)
